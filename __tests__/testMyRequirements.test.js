...
describe('Tela de meus requerimentos', () => {
    ...
    it('Seta deve voltar para a tela de perfil', async () => {
        const { getByTestId } = render(
            <NavigationContainer>
              <MyRequirements navigation={mockNavigation} />
            </NavigationContainer>
        );
    ...
    it('Deve renderizar dois requerimentos', async () => {
        await AsyncStorage.setItem('user', JSON.stringify({registration: 2021004000}));
    ...
    it('Clicar no requerimento deve abrir a tela de informações do requerimento', async () => {
        ...
        await waitFor(() => {
            ...
            expect(mockNavigation.navigate).toHaveBeenCalledWith('RequirementInfo');
        });
    });
});